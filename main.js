...
function countdown() {
    ...
    if (countdownFrom == 2) {
        ...
        drawNumber('Я люблю тебя :*');
    }
    if (countdownFrom == 0) {
        countdownRunning = true;
        // Now that countdowns finised show the text Go
        drawNumber('Ты самая лучшая)');
    }
    if (countdownFrom == -1) {
        countdownRunning = false;
        // Now that countdowns finised show the text Go
        drawNumber('С днем рождения)');
    }
...
function drawNumber(num) {
    ...
    numberStageCtx.font = "bold 140px Lato";
    numberStageCtx.fillText(num, 600, 250);
